...
template <typename T> void mapCleanUp( std::map<duint32, T*>& table )
{
    for( auto & item : table )
        delete item.second;
}
...
bool dwgReader::readDwgTables( DRW_Header& hdr, dwgBuffer* dbuf )
{
    ...
    else
    {
        ...
        for( std::list<duint32>::iterator it = dimstyControl.hadlesList.begin(); it != dimstyControl.hadlesList.end();
                ++it )
        {
            mit = ObjectMap.find( *it );
            if( mit == ObjectMap.end() )
    ...
    else
    {
        ...
        for( std::list<duint32>::iterator it = vportControl.hadlesList.begin(); it != vportControl.hadlesList.end();
                ++it )
        {
            mit = ObjectMap.find( *it );
            if( mit == ObjectMap.end() )
    ...
    else
    {
        ...
        for( std::list<duint32>::iterator it = blockControl.hadlesList.begin(); it != blockControl.hadlesList.end();
                ++it )
        {
            mit = ObjectMap.find( *it );
            if( mit == ObjectMap.end() )
    ...
    else
    {
        ...
        for( std::list<duint32>::iterator it = appIdControl.hadlesList.begin(); it != appIdControl.hadlesList.end();
                ++it )
        {
            mit = ObjectMap.find( *it );
            if( mit == ObjectMap.end() )
