...
public:
    ...
    virtual duint64 size()
    {
        return sz;
    }
    virtual duint64 getPos()
    {
        return stream->tellg();
    }
    ...
    virtual dwgBasicStream* clone()
    {
        return new dwgFileStream( stream );
    }
...
public:
    ...
    virtual dwgBasicStream* clone()
    {
        return new dwgCharStream( stream, sz );
    }
...
public:
    ...
    UTF8STRING getVariableText( DRW::Version v,
                                bool nullTerm = true ); // TV => call TU for 2007+ or T for previous versions
