...
bool dwgReader21::parseSysPage( duint64 sizeCompressed,
...
bool dwgReader21::readFileHeader()
{
    ...
    else
    {
        ...
        fileHdrData.resize( fileHdrDataLength );
        dwgCompressor::decompress21( compByteStr.data(), &fileHdrData.front(), fileHdrCompLength, fileHdrDataLength );
    }
    ...
    bool ret = parseSysPage( PagesMapSizeCompressed,
                             PagesMapSizeUncompressed,
                             PagesMapCorrectionFactor,
                             0x480 + PagesMapOffset,
                             &PagesMapData.front() );
    if( !ret )
        return false;
    ...
    ret = parseSysPage( SectionsMapSizeCompressed,
                        SectionsMapSizeUncompressed,
                        SectionsMapCorrectionFactor,
                        sectionMap.address,
                        &SectionsMapData.front() );
    if( !ret )
        return false;
